import { none, type Option, some } from "@bearz/option";
...
export class OrderedMap<K, V> extends Map<K, V> {
    ...
    override values(): MapIterator<V> {
        return this.#keys.map((key) => this.get(key) as V)[Symbol.iterator]();
    }

    // @ts-ignore TS2552
    override entries(): MapIterator<[K, V]> {
        return this.#keys.map((key) => [key, this.get(key)] as [K, V])[Symbol.iterator]();
    }
