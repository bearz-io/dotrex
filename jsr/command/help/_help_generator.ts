...
import { dedent, getDescription, getFlag, parseArgumentsDefinition } from "../_utils.ts";
    ...
    private generateHints(option: Option): string {
        ...
        if (typeof option.default !== "undefined") {
            ...
            if (typeof defaultValue !== "undefined") {
                hints.push(
                    bold(`Default: `) + inspect(defaultValue, { colors: this.options.colors }),
                );
            }
        }
                ...
                hints.push(
                    bold(`Values: `) +
                        possibleValues.map((value: unknown) =>
                            inspect(value, { colors: this.options.colors })
                        ).join(", "),
                );
            }
        }
