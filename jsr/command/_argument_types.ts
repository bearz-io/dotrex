...
import type { CamelCase, Merge, MergeRecursive, TrimLeft, TrimRight } from "./_type_utils.ts";
...
> = TArg extends RestArgsListTypeCompletion<infer Type>
    ...
    : TArg extends SingleArgType<infer Type> ? TTypes extends Record<Type, infer R> ? R : unknown
...
> = number extends TTypes ? any
    : TFlags extends `${string}--${infer Name}=${infer TRestFlags}` ? ValuesOption<
            Name,
            TRestFlags,
            TTypes,
...
> = OptionName<TName> extends infer Name extends string
    ? TDefault extends undefined ? Name extends keyof TOptions ? { [Key in Name]?: false }
        : { [Key in Name]: boolean }
