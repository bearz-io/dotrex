 ...
 * - `<name:type[]:completion>`
 */
type SingleArgListTypeCompletion<TType extends string> = OptionalOrRequiredValue<
    `${string}:${TType}[]:${string}`
>;
/**
...
type ArgumentType<TArg extends string, TCustomTypes, TTypes = Merge<DefaultTypes, TCustomTypes>> =
    TArg extends RestArgsListTypeCompletion<infer Type>
        ...
        : unknown;
type ArgumentTypes<TFlags extends string, T> = TFlags extends `${string} ${string}`
    ? TypedArguments<TFlags, T>
    : ArgumentType<TFlags, T>;
export type TypedArguments<TArgs extends string, TTypes> = number extends TTypes ? any
    ...
    : TArgs extends `${string}...${string}` ? [
            ...ArgumentType<TArgs, TTypes> extends Array<infer TValue>
                ? TArgs extends `[${string}]` ? Array<TValue> : [TValue, ...Array<TValue>]
                : never,
        ]
    : TArgs extends `[${string}]` ? [ArgumentType<TArgs, TTypes>?]
    : [ArgumentType<TArgs, TTypes>];
export type TypedCommandArguments<TNameAndArguments extends string, TTypes> = number extends TTypes
    ? any
    : TNameAndArguments extends `${string} ${infer TFlags}` ? TypedArguments<TFlags, TTypes>
    : [];
...
> = number extends TTypes ? any
    : TFlags extends `${string}--${infer Name}=${infer TRestFlags}`
        ? ValuesOption<Name, TRestFlags, TTypes, IsRequired<TRequired, TDefault>, TDefault>
    : TFlags extends `${string}--${infer Name} ${infer TRestFlags}`
        ? ValuesOption<Name, TRestFlags, TTypes, IsRequired<TRequired, TDefault>, TDefault>
    : TFlags extends `${string}--${infer Name}`
        ? BooleanOption<Name, TOptions, IsRequired<TRequired, TDefault>, TDefault>
    : TFlags extends `-${infer Name}=${infer TRestFlags}`
        ? ValuesOption<Name, TRestFlags, TTypes, IsRequired<TRequired, TDefault>, TDefault>
    : TFlags extends `-${infer Name} ${infer TRestFlags}`
        ? ValuesOption<Name, TRestFlags, TTypes, IsRequired<TRequired, TDefault>, TDefault>
...
type NegatableOption<TName extends string, TOptions, TDefault> = OptionName<TName> extends
    infer Name extends string ? TDefault extends undefined ? Name extends keyof TOptions ? {
                [Key in Name]?: false;
            }
        : {
            [Key in Name]: boolean;
...
type ValueOption<
    ...
    TRequired extends boolean | undefined = undefined,
    TDefault = undefined,
> = TName extends `${infer Name}.${infer RestName}` ? (TRequired extends true ? {
    ...
    : (TRequired extends true ? {
            [Key in OptionName<TName>]: ExtractArgumentsFromFlags<TRestFlags> extends `[${string}]`
                ?
...
export type MergeOptions<TFlags, TOptions, TMappedOptions, TName = GetOptionName<TFlags>> =
    TName extends `no-${string}` ? Spread<TOptions, TMappedOptions>
        : TName extends `${string}.${string}` ? MergeRecursive<TOptions, TMappedOptions>
        : Merge<TOptions, TMappedOptions>;
export type MapValue<TOptions, TMappedOptions, TCollect = undefined> = TMappedOptions extends
    undefined ? TCollect extends true ? {
            [Key in keyof TOptions]: TOptions[Key] extends (Record<string, unknown> | undefined)
...
type ExtractArgumentsFromFlags<TFlags extends string> = TFlags extends
    ...
    : TFlags extends `-${string} ${infer RestFlags}` ? ExtractArgumentsFromFlags<RestFlags>
    : TFlags;
type OptionName<Name extends string> = Name extends "*" ? string : CamelCase<TrimRight<Name, ",">>;
type IsRequired<TRequired extends boolean | undefined, TDefault> = TRequired extends true ? true
