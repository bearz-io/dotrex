...
type SingleArgListTypeCompletion<TType extends string> =
  OptionalOrRequiredValue<`${string}:${TType}[]:${string}`>;
/**
...
type ArgumentTypes<TFlags extends string, T> = TFlags extends
  `${string} ${string}` ? TypedArguments<TFlags, T> : ArgumentType<TFlags, T>;
...
type NegatableOption<TName extends string, TOptions, TDefault> =
  OptionName<TName> extends infer Name extends string
    ? TDefault extends undefined ? Name extends keyof TOptions ? {
          [Key in Name]?: false;
        }
      : {
        [Key in Name]: boolean;
...
type IsRequired<TRequired extends boolean | undefined, TDefault> =
  TRequired extends true ? true : TDefault extends undefined ? false : true;
export {};
