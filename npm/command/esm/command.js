...
export class Command {
  ...
  type(name, handler, options) {
    ...
    if (
      ...
      const completeHandler = (cmd, parent) =>
  ...
  parseOptions(
    ctx,
    options,
    { stopEarly = this._stopEarly, stopOnUnknown = false, dotted = true } = {},
  ) {
    parseFlags(ctx, {
      stopEarly,
      stopOnUnknown,
    ...
    const getGlobalOption = (parent, noGlobals) => {
      ...
      if (!option?.global) {
        return parent._parent &&
          getGlobalOption(parent._parent, noGlobals || parent._noGlobals);
      }
      if (noGlobals && option !== helpOption) {
        return;
      }
