...
export class NpmProvider extends Provider {
    ...
    async getVersions(name) {
        const response = await fetch(new URL(this.#getPackageName(name), this.apiUrl));
        if (!response.ok) {
    ...
    getRepositoryUrl(name, version) {
        return new URL(`package/${this.#getPackageName(name)}${version ? `/v/${version}` : ""}`, this.repositoryUrl).href;
    }
    getRegistryUrl(name, version) {
        return `npm:${this.#getPackageName(name)}@${version}`;
    }
    #getPackageName(name) {
        return `${this.packageScope ? `@${this.packageScope}/` : ""}${this.packageName ?? name}`;
    }
}
