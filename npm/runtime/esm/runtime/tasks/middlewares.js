import { CyclicalTaskReferences, MissingTaskDependencies, TaskCancelled, TaskCompleted, TaskFailed, TaskSkipped, TaskStarted, } from "./messages.js";
import { TaskPipelineContext, TaskPipelineMiddleware, TasksPipelineMiddleware, } from "./pipelines.js";
...
export class SequentialTaskExecution extends TasksPipelineMiddleware {
    async run(ctx, next) {
        ...
        for (const task of taskSet) {
            ...
            if (ctx.status === PipelineStatuses.Failed || ctx.status === PipelineStatuses.Cancelled) {
