        ...
        try {
            ...
            if (!mod.deployments) {
                if (globalDeployments.size === 0) {
                    ctx.bus.debug(`No deployments found in ${file}.  Rexfile file must export a variable called deployments is type DeploymentMap.`);
                }
                else {
                ...
                if (!(mod.deployments instanceof DeploymentMap)) {
                    ctx.bus.error(new Error(`Deployments in ${file} must be of type DeploymentMap`), "Error collecting tasks");
                    return;
                }
            ...
            if (!mod.jobs) {
                if (globalJobs.size === 0) {
                    ctx.bus.debug(`No jobs found in ${file}.  Rexfile file must export a variable called jobs is type JobMap.`);
                }
                else {
