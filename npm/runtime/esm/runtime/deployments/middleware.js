...
import { DeploymentCancelled, DeploymentCompleted, DeploymentFailed, DeploymentSkipped, DeploymentStarted, } from "./messages.js";
            ...
            model.eventHandlers[key] = async (c) => {
                ...
                for (const [key, value] of tasksCtx.secrets) {
                    if (ctx.secrets.has(key)) {
                        ...
                        if (old !== value) {
                            ctx.writer.debug(`Secret ${key} has changed in deployment ${c.model.id}`);
