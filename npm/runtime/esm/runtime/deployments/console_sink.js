...
export function deployConsoleSink(message) {
  switch (message.kind) {
    case "deployment:missing-dependencies": {
      const msg = message;
      writer.error(
        `Missing the following deployment dependencies ${
          msg.deployments.join(",")
        }`,
      );
      return;
    }
    case "deployment:cyclical-references": {
      const msg = message;
      writer.error(
        `Found deployment cyclical references ${msg.deployments.join(",")}`,
      );
      return;
    }
    ...
    case "deployment:completed": {
      const msg = message;
      const duration = msg.result.finishedAt.getTime() -
        msg.result.startedAt.getTime();
