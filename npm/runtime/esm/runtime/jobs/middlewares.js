...
import { CyclicalJobReferences, JobCancelled, JobCompleted, JobFailed, JobSkipped, JobStarted, MissingJobDependencies, } from "./messages.js";
...
export class JobsExcution extends JobsPipelineMiddleware {
    async run(ctx, next) {
        ...
        if (cyclesRes.length > 0) {
            ...
            ctx.error = new Error(`Cyclical job references found: ${cyclesRes.map((o) => o.id).join(", ")}`);
