...
import {
  JobsPipelineContext,
  SequentialJobsPipeline,
} from "./jobs/pipelines.js";
...
import {
  ...
  RunDeployment,
} from "./deployments/middleware.js";
...
export class Runner {
  ...
  async run(options) {
    ...
    try {
      ...
      if (
        res.tasks.size === 0 && res.jobs.size === 0 &&
        res.deployments.size === 0
      ) {
        ...
        case "deploy":
          {
            if (targets.length > 1) {
              writer.error(
                "Deploy command does not support multiple targets yet.",
              );
              exit(1);
            }
            ...
            const deploymentsCtx = new DeploymentPipelineContext(
              ctx,
              deployment,
              {
                id: deployment.id,
