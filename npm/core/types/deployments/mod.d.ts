...
export declare class DeploymentEventError extends Error {
    ...
    constructor(message: string, event: DeploymentEvent, result: DeploymentEventResult, options?: ErrorOptions);
}
...
export type DeploymentEvent = "before:deploy" | "after:deploy" | "before:destroy" | "after:destroy" | "before:rollback" | "after:rollback" | string;
...
export type DeployDelegate = (ctx: DeploymentContext) => Promise<Outputs> | Promise<void> | Outputs | void;
...
export declare function deploy(id: string, needs: string[], run: DeployDelegate, tasks: DeploymentMap): Deployment;
