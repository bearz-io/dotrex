...
const groupSymbol = "\x1b[38;2;60;0;255m❯\x1b[39m\x1b[38;2;90;0;255m❯\x1b[39m\x1b[38;2;121;0;255m❯\x1b[39m\x1b[38;2;151;0;255m❯\x1b[39m\x1b[38;2;182;0;255m❯\x1b[39m";
        ...
        switch (directive) {
            case "destroy": {
                if (ctx.model.eventHandlers["before:destroy"]) {
                    ...
                    if (r.error || r.status === PipelineStatuses.Failed ||
                        r.status === PipelineStatuses.Cancelled) {
                        throw new DeploymentEventError(`Deployment ${this.id} failed during before:destroy tasks`, "before:destroy", r, { cause: r.error });
                    }
                }
                ...
                if (AnsiSettings.current.mode === AnsiModes.TwentyFourBit) {
                    ...
                    writer?.writeLine(` ${this.name ?? this.id} completed sucessfully in ${green(m.toString())}m ${green(s.toString())}s ${green(ms.toString())}ms`);
                }
                else {
                    writer?.success(`${this.name ?? this.id} completed in ${m}m ${s}s ${ms}ms`);
                }
                writer?.endGroup();
                if (ctx.model.eventHandlers["after:destroy"]) {
                    ...
                    if (r.error || r.status === PipelineStatuses.Failed ||
                        r.status === PipelineStatuses.Cancelled) {
                        throw new DeploymentEventError(`Deployment ${this.id} failed during after:destroy tasks`, "after:destroy", r, { cause: r.error });
                    }
                }
                return res;
            }
            case "rollback": {
                if (ctx.model.eventHandlers["before:rollback"]) {
                    ...
                    if (r.error || r.status === PipelineStatuses.Failed ||
                        r.status === PipelineStatuses.Cancelled) {
                        throw new DeploymentEventError(`Deployment ${this.id} failed during before:rollback tasks`, "before:rollback", r, { cause: r.error });
                    }
                }
                ...
                if (ctx.model.eventHandlers["after:rollback"]) {
                    ...
                    if (r.error || r.status === PipelineStatuses.Failed ||
                        r.status === PipelineStatuses.Cancelled) {
                        throw new DeploymentEventError(`Deployment ${this.id} failed during after:rollback tasks`, "after:rollback", r, { cause: r.error });
                    }
                }
                return res;
            }
            ...
            default: {
                if (ctx.model.eventHandlers["before:deploy"]) {
                    ...
                    if (r.error || r.status === PipelineStatuses.Failed ||
                        r.status === PipelineStatuses.Cancelled) {
                        throw new DeploymentEventError(`Deployment ${this.id} failed during before:deploy tasks`, "before:deploy", r, { cause: r.error });
                    }
                }
                ...
                if (ctx.model.eventHandlers["after:deploy"]) {
                    ...
                    if (r.error || r.status === PipelineStatuses.Failed ||
                        r.status === PipelineStatuses.Cancelled) {
                        throw new DeploymentEventError(`Deployment ${this.id} failed during after:deploy tasks`, "after:deploy", r, { cause: r.error });
                    }
                }
                return res;
            }
